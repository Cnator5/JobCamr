...
//     const { userProfile } = useGlobalContext(); // Ensure correct access

//     const [jobs, setJobs] = useState([]); //jobs will be an array of job objects
//     const [loading, setLoading] = useState(false);
//     const [userJobs, setUserJobs] = useState([]); //jobs will be an array of job objects
...
//         try {
//             const res = await axios.post('/api/v1/jobs', jobData); // Use jobData
...
//             //send the request
//             const res = await axios.get(`/api/v1/jobs/search?${query.toString()}`); //query.toString() is optional
...
//         setLoading(true);
//         try {
//             const res = await axios.get(`/api/v1/jobs/${id}`); // Use id
...
//             getJobs();
//         } catch (error) {
//             console.log("Error liking jobs", error);
//         }
//     };
...
//             setJobs((prevJobs) => prevJobs.filter((job) => job._id !== jobId)); // Update jobs state
//             setUserJobs((prevUserJobs) => prevUserJobs.filter((job) => job._id !== jobId)); // Update userJobs state
